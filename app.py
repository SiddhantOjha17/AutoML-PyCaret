...
if choice == "Modelling":
    ...
    if choice=='Regression':
        ...
        if st.button('Run Modelling'):
            pycaret.regression.setup(df, target=chosen_target, train_size=split_size,fold = 2)
    ...
    if choice=='Classification':
        ...
        chosen_target = st.selectbox('Choose the Target Column', df.columns)
        if st.button('Run Modelling'):
            pycaret.classification.setup(df, target=chosen_target, train_size=split_size,fold = 2)
